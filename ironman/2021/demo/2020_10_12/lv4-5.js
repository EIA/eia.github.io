...
app.ticker.add((delta) => {
    ...
    let targetSpeed, targetFrequency;
    if (isPress === true){
    ...
    emitter1.updateOwnerPos(
        app.screen.width / 2 + Math.cos(rot1) * radius,
        app.screen.height / 2 + Math.sin(rot1) * radius
    );

    const rot2 = emitter2.rotation / (180 / Math.PI);
    emitter2.updateOwnerPos(
        app.screen.width / 2 + Math.cos(rot2) * radius,
        app.screen.height / 2 + Math.sin(rot2) * radius
    );

    const rot3 = emitter3.rotation / (180 / Math.PI);
    emitter3.updateOwnerPos(
        app.screen.width / 2 + Math.cos(rot3) * radius,
        app.screen.height / 2 + Math.sin(rot3) * radius
    );

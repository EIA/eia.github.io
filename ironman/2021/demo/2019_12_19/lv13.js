...
createAllBox(currentInteractiveTarget);
  ...
  container.children.forEach((child, index)=>{
    ...
    if(child === container.children[2]){
      TweenMax.to(child,  { duration:.3, pixi: { tint: 0xff9900 }, ease:Strong.easeOut  });
      // gsap.to(child,  { duration:.3, pixi: { tint: 0xff9900 }, ease:Strong.easeOut  });
    }else{
      // gsap.to(child,  { duration:.3, pixi: { tint: 0x0 }, ease:Strong.easeOut  });
    }
  })
...
function overHandler(target){
  // target.tint = 0xff9900;
  // TweenMax.to(target, .3, { tint: 0xff9900, ease:Strong.easeOut  });
  gsap.to(target,  { duration:.3, pixi: { tint: 0xff9900 }, ease:Strong.easeOut  });
...
function touchHandler(){
  // console.log('touchHandler', currentInteractiveTarget, currentInteractiveTarget.name)
  container.children.forEach((child, index)=>{
    // console.log(child.name)
    if(child === currentInteractiveTarget){
      TweenMax.to(child,  .3, { pixi: { tint: 0xff9900 }, ease:Strong.easeOut  });
    }else{
      TweenMax.to(child,  .3, { pixi: { tint: 0x0 }, ease:Strong.easeOut  });
    }
  })
}
